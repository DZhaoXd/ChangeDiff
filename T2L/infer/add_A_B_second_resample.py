...
def modify_percentages(random_string, N=8):
    ...
    class_name = ['water', 'ground', 'low vegetation', 'tree', 'building', 'sports field']
...
source_num = 2000
add_num = 4
save_path = "./sample_results/add_A_B_resample/"
# 创建目标文件夹
if not os.path.exists(save_path):
    os.makedirs(save_path)

## source json
source_base_path = 'train/data/second/train'
source_txt_json_path = 'train/data/second/train/metadata.jsonl'
...
## prompt json
prompt_json_path = 'train/data/second_r1/train/metadata.jsonl'
...
for num, random_element in enumerate(random_elements):
    ...
    ori_layout_a = np.array(Image.open(ori_layout_a_path))
    ori_layout_a = map_colors_to_ids(ori_layout_a)
    
    ori_layout_b = np.array(Image.open(ori_layout_b_path))
    ...
    replace_id = random.sample([1, 2, 3, 4], 1)[0]
    ori_replace = False
    prompt_success = True
    ...
    for add_id in range(add_num):
        try:
            if add_id == 0:
                ...
                prompt, ratio_state = modify_percentages(A_prompt, N=5)
                prompt, class_state = add_random_class(class_name, prompt)
                if class_state is not None:
                    state = class_state
                else:
                    state = ratio_state
                prompt = "A remote sensing image of " + prompt
        ...
        image_id_numpy[image_id_numpy>5] = replace_id
        ...
        if ori_replace:
            if add_id == 0:
                image_id_numpy[ori_layout_a<=5] = ori_layout_a[ori_layout_a<=5]
            else:
                image_id_numpy[ori_layout_b<=5] = ori_layout_b[ori_layout_b<=5]
        ###### 

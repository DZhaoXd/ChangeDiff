...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, file_name='SECOND', data_root='data/', transform=None, json_file=None,mode='all'):
        ...
        if file_name == 'SECOND':
            ...
            self.height = 416
            self.width = 416
        elif file_name == 'CNAM-CD_V1':
            self.num_classes = 6
            self.ST_COLORMAP = [[255, 255, 255], [128, 128, 128], [0, 128, 0], [0, 255, 0], [0, 0, 255],[255,0,0]]
            self.ST_CLASSES = ['No change', 'Impervious_surface', 'bare_ground', 'vegetation', 'Water','Other']
            self.height = 512
            self.width = 512
        ...
        # if dataset == 'train':
        self.dataset_info = json.load(open(data_root +"/" + json_file, 'r'))
        # else:
        #     self.dataset_info = json.load(open(data_root + '/' + dataset +'/'+ 'val.json', 'r'))
    ...
    def __getitem__(self, idx):
        ...
        if pre_image_path.endswith("tif"):
            pre_label = self.load_scd(pre_label_path)
            post_label = self.load_scd(post_label_path)
        else:
            pre_label = self.load(pre_label_path)
            post_label = self.load(post_label_path)
        if pre_image_path.endswith("png"):
            pre_label = Color2Index(pre_label, self.colormap2label, self.num_classes).astype(np.uint8)
            post_label = Color2Index(post_label, self.colormap2label, self.num_classes).astype(np.uint8)
        ...
        # if pre_image_path.endswith("png"):
        #     unique_elements = torch.unique(pre_label_tensor)
...
if __name__ == '__main__':
    ...
    json_file = 'PSDImage_label_no_ignore_3-4/train_concat_3-4.json'
    ...
    for idata in range(len(dataset)):
        data = dataset.__getitem__(len(dataset)-1-idata)
        pre_label=data[2].numpy()
        post_label=data[3].numpy()
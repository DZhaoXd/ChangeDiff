...
def Color2Index(ColorLabel, colormap2label=None, num_classes=None):
    ...
    label_out = np.zeros(ColorLabel.shape[:2])
    h, w = ColorLabel.shape[:2]
    for i in range(h):
        for j in range(w):
            if ColorLabel[i][j] == 255:
                label_out[i][j] = 1
            else:
                label_out[i][j] = 0
    return label_out
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, file_name='SECOND', data_root='data/', transform=None, json_file=None,mode='all'):
        ...
        if file_name == 'SECOND':
            ...
            self.height = 512
            self.width = 512
            # self.dataset_info = json.load(open(json_file, 'r'))
            ...
            self.num_classes = 2
            self.ST_COLORMAP = [[0, 0, 0], [255, 255, 255]]
            self.ST_CLASSES = ['nobuilding', 'building']
            self.height = 512
            self.width = 512
            self.dataset_info = json.load(open(json_file, 'r'))
        ...
        for data_sample in self.dataset_info[mode + '_set']:
            ...
            self.change_gts.append(data_sample[4])
        ...
        if transform:
            ...
            ], additional_targets={'image1': 'image', 'mask1': 'mask', 'change': 'mask'})
    ...
    def __getitem__(self, idx):
        ...
        change_label_path = self.change_gts[idx]
        #
        ...
        change_label = self.load(change_label_path)
        ...
        # change_label = Color2Index(change_label, self.colormap2label, self.num_classes)
        #
        if self.transform:
            sample = self.train_transforms_all(image=pre_image, image1=post_image, mask=pre_label, mask1=post_label, change=change_label)
            pre_image, post_image, pre_label, post_label, change_label = (
                sample['image'], sample['image1'], sample['mask'], sample['mask1'], sample['change'])
        ...
        sample = self.to_tensor(image=pre_image, image1=post_image, mask=pre_label, mask1=post_label, change=change_label)
        pre_image_tensor, post_image_tensor, pre_label_tensor, post_label_tensor, change_label_tensor = sample['image'].contiguous(), \
            sample['image1'].contiguous(), sample['mask'].contiguous(), sample['mask1'].contiguous(), sample['change'].contiguous()

        return pre_image_tensor, post_image_tensor, \
            pre_label_tensor.unsqueeze(dim=0), post_label_tensor.unsqueeze(dim=0), change_label_tensor.unsqueeze(dim=0), self.file_list[idx]
...
if __name__ == '__main__':
    ...
    dataset = Dataset('train', file_name=file_name, data_root=data_root, transform=True, json_file=json_file,mode='train')

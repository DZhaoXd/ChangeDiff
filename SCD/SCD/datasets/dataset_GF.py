...
def label_map(label,guofeng_label_mapping):
    label_out = np.zeros(label.shape[:2])
    for k, v in guofeng_label_mapping.items():
        label_out[label == k] = v
    return label_out
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, file_name='SECOND', data_root='data/', transform=None, json_file=None,mode='all'):
        ...
        if file_name == 'SECOND':
            ...
            self.num_classes = 9
            self.ST_COLORMAP = [[255, 255, 255], [0, 0, 255], [0, 0, 128], [128, 0, 0], [255, 128, 0], [0, 128, 0], [0, 255, 0], [128, 128, 128], [0, 255, 255]]
            self.ST_CLASSES = ['Ignore', 'Water bodies', 'transportation', 'building', 'farmland','lawn','woodland','bare soil','Other']
            self.height = 512
            self.width = 512
        self.colormap2label = np.zeros(256 ** 3)
        self.guofeng_label_mapping = {
            0: 0,
            1: 1,
            2: 0,
            3: 5,
        ...
        # if dataset == 'train':
        # self.dataset_info = json.load(open(data_root + '/' + 'train' + '/' + json_file, 'r'))
    ...
    def __getitem__(self, idx):
        ...
        pre_label = label_map(pre_label,self.guofeng_label_mapping)
        post_label = Color2Index(post_label, self.colormap2label, self.num_classes)
        post_label = label_map(post_label, self.guofeng_label_mapping)
        #
        if self.transform:
